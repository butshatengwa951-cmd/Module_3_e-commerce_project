  ...
  <div class="catalogue-page">
    <section class="catalogue-hero">
      <div>
        ...
        <p class="hero-copy">
          Compare supplier prices, choose your best deal and explore your
          stokvel's bulk-shopping options.
        </p>
      </div>
    ...
    <div v-if="selectedCategory !== 'All Products'" class="active-filter">
      ...
      <button type="button" @click="selectedCategory = 'All Products'">×</button>
    </div>
...
const basketCount = ref(Number(localStorage.getItem("basketCount") || 0));

const categories = [
  "All Products", "Staples", "Cooking Essentials", "Food",
  "Canned Food", "Spices", "Breakfast", "Beverages",
];
...
function getCategoryIcon(category) {
  return {
    "All Products": "🛍️", Staples: "🌾", "Cooking Essentials": "🍳",
    Food: "🍎", "Canned Food": "🥫", Spices: "🌶️", Breakfast: "🥣", Beverages: "☕",
  }[category] || "🛍️";
}
...
function getLowestPrice(product) {
  if (!product.supplier_prices?.length) return 0;
  return Math.min(...product.supplier_prices.map((supplier) => Number(supplier.price)));
}

function getHighestPrice(product) {
  if (!product.supplier_prices?.length) return 0;
  return Math.max(...product.supplier_prices.map((supplier) => Number(supplier.price)));
}

function getSaving(product) {
  if (product.supplier_prices?.length < 2) return 0;
...
function getCheapestSupplier(product) {
  ...
  return product.supplier_prices.reduce((lowest, supplier) =>
    Number(supplier.price) < Number(lowest.price) ? supplier : lowest
  ).supplier_name;
}
...
function addToBasket(product) {
  ...
  const existing = basket.find((item) => item.product_id === product.product_id);
  ...
  basketCount.value = basket.reduce((total, item) => total + Number(item.quantity || 0), 0);
  ...
  message.value = `${product.product_name} added to your group basket.`;
...
const filteredProducts = computed(() => {
  let result = products.value.filter((product) => {
    ...
    return name.toLowerCase().includes(search.value.toLowerCase()) &&
      (selectedCategory.value === "All Products" || category === selectedCategory.value);
  });
...
.eyebrow, .category-title > span, .basket-summary > span, .best-supplier > span {
...
h1 span { color: var(--sw-gold-500); }
.hero-copy { max-width: 620px; color: var(--sw-page-text); opacity: .78; font-size: 16px; line-height: 1.7; }

.basket-summary, .shop-toolbar, .category-panel, .product-card, .empty-state {
...
.basket-summary { min-width: 190px; padding: 20px; border-radius: 18px; display: grid; gap: 5px; }
.basket-summary strong { font-size: 38px; }
.basket-summary small { opacity: .65; }
.basket-summary a { color: var(--sw-gold-500); margin-top: 8px; text-decoration: none; font-weight: 700; }
...
.shop-toolbar button, .shop-toolbar select, .search-box, .category-list button, .add-button, .compare-button, .empty-state button {
  border: 1px solid var(--sw-glass-border); border-radius: 12px; font: inherit;
}
...
.search-box { display: flex; align-items: center; gap: 10px; padding: 0 14px; background: var(--sw-input-background); }
.search-box input { width: 100%; border: 0; outline: 0; background: transparent; color: inherit; padding: 12px 0; }

.category-panel { max-width: 1180px; margin: 0 auto 18px; padding: 18px; border-radius: 16px; }
.category-title { display: grid; gap: 4px; margin-bottom: 12px; }
.category-title small { opacity: .65; }
.category-list { display: flex; flex-wrap: wrap; gap: 8px; }
.category-list button { padding: 10px 14px; background: var(--sw-input-background); color: inherit; cursor: pointer; }
.category-list button.active { border-color: var(--sw-gold-500); color: var(--sw-gold-500); }
.active-filter { max-width: 1180px; margin: 0 auto 14px; font-size: 14px; }
.active-filter button { border: 0; background: transparent; color: inherit; cursor: pointer; font-size: 20px; margin-left: 5px; }

.products-area { max-width: 1180px; margin: 0 auto; }
.product-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(270px, 1fr)); gap: 18px; }
.product-card { border-radius: 20px; overflow: hidden; }
.product-image { height: 240px; position: relative; display: grid; place-items: center; padding: 20px; background: rgba(255,255,255,.04); }
.product-image img { max-width: 100%; max-height: 205px; object-fit: contain; }
.deal-badge, .bulk-badge { position: absolute; top: 12px; padding: 6px 9px; border-radius: 999px; font-family: "DM Mono", monospace; font-size: 9px; z-index: 1; }
.deal-badge { left: 12px; background: var(--sw-gold-500); color: #171022; }
.bulk-badge { right: 12px; background: var(--sw-purple-700); color: white; }
.product-info { padding: 20px; }
.category-label { color: var(--sw-gold-500); font-family: "DM Mono", monospace; font-size: 10px; text-transform: uppercase; }
.product-info h2 { margin: 7px 0; font-size: 21px; }
.description { min-height: 42px; opacity: .7; font-size: 13px; line-height: 1.5; }
.price-row { display: flex; justify-content: space-between; align-items: end; margin: 18px 0; }
.price-row small { display: block; opacity: .6; font-size: 11px; }
.price-row strong { font-size: 25px; }
.saving { color: var(--sw-gold-500); font-size: 12px; font-weight: 700; }
.best-supplier { display: grid; gap: 3px; padding: 12px; border-radius: 12px; background: rgba(121,93,137,.14); }
.best-supplier strong { font-size: 14px; }
.stock-row { display: flex; justify-content: space-between; margin: 12px 0; font-size: 12px; opacity: .8; }
.in-stock { color: #68d391; }.low-stock { color: var(--sw-terra-500); }
.compare-button, .add-button { width: 100%; padding: 11px 13px; cursor: pointer; color: inherit; }
.compare-button { display: flex; justify-content: space-between; background: transparent; }
.add-button { margin-top: 10px; background: var(--sw-button-gradient); color: white; font-weight: 700; }
.supplier-list { display: grid; gap: 7px; margin-top: 9px; }
.supplier-row { display: flex; justify-content: space-between; padding: 10px; border-radius: 10px; background: rgba(255,255,255,.04); font-size: 12px; }
.supplier-row div { display: grid; gap: 2px; }.supplier-row small { opacity: .6; }.supplier-row.cheapest { border: 1px solid var(--sw-gold-500); }

.state, .empty-state { min-height: 260px; border-radius: 18px; display: grid; place-items: center; align-content: center; gap: 8px; text-align: center; padding: 30px; }
.state p, .empty-state p { opacity: .7; }.error { border-color: var(--sw-terra-500); }
.empty-state > div { font-size: 48px; }.empty-state h2 { margin: 0; }.empty-state button { padding: 10px 15px; background: var(--sw-button-gradient); color: white; cursor: pointer; }
.loader { width: 34px; height: 34px; border: 3px solid var(--sw-glass-border); border-top-color: var(--sw-gold-500); border-radius: 50%; animation: spin .8s linear infinite; }
.toast { position: fixed; right: 20px; bottom: 20px; z-index: 20; max-width: 360px; padding: 14px 16px; border-radius: 12px; background: var(--sw-purple-700); color: white; box-shadow: var(--sw-shadow-soft); }
.category-slide-enter-active, .category-slide-leave-active { transition: .2s ease; }.category-slide-enter-from, .category-slide-leave-to { opacity: 0; transform: translateY(-8px); }
