...
{path:"/login-signup",name:"AuthSelector",component:AuthSelector},{path:"/login",name:"Login",component:Login},{path:"/signup",name:"Signup",component:Signup},{path:"/forgot-password",name:"ForgotPassword",component:ForgotPassword},{path:"/reset-password",name:"ResetPassword",component:ResetPassword}];
const router=createRouter({history:createWebHistory(),routes});function hasAuthToken(){return Boolean(localStorage.getItem("token")||localStorage.getItem("sw_token"))}function isAdmin(){try{return JSON.parse(localStorage.getItem("user")||"null")?.role==="admin"}catch{return false}}
router.beforeEach((to)=>{if(to.meta.requiresAuth&&!hasAuthToken())return{path:"/login-signup",query:{redirect:to.fullPath}};if(to.meta.requiresAdmin&&!isAdmin())return{path:"/",query:{admin:"forbidden"}};return true});export default router;
