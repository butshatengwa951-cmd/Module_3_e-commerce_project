<template>
  <header class="sw-header">
    ...
    <div class="header-actions">
      ...
      <template v-else>
        <router-link class="join-btn" to="/">Join</router-link>
  ...
  <nav v-if="menuOpen" class="mobile-nav" aria-label="Mobile navigation">
    ...
    <router-link to="/" @click="menuOpen = false">Join</router-link>
...
.sw-header {
  ...
  align-items: center;
  gap: 28px;
  background: color-mix(in srgb, var(--sw-page-background) 94%, transparent);
...
.brand-mark {
  ...
  place-items: center;
  font-size: 22px;
  border: 2px solid var(--sw-gold-500);
}
