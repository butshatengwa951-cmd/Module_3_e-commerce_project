...
export const getPendingOrderForStokvel = async (stokvelId) => {
  const [orders] = await pool.query(
    `
      SELECT od.order_id, od.user_id, od.stokvel_id, od.card_id, od.delivery_id,
             od.order_date, od.total_amount, od.order_status, s.stokvel_name
  ...
  const [items] = await pool.query(
    `
      SELECT oi.order_item_id, oi.order_id, oi.product_id, oi.supplier_price_id,
             oi.quantity, oi.unit_price, oi.subtotal, p.product_name, p.image_url,
             sp.supplier_name
      FROM order_items oi
...
export const getOrderHistoryForStokvel = async (stokvelId) => {
  const [orders] = await pool.query(
    `
      SELECT od.order_id, od.user_id, od.stokvel_id, od.card_id, od.delivery_id,
             od.order_date, od.total_amount, od.order_status, cd.card_type,
             cd.last_four_digits, dd.delivery_status, COUNT(oi.order_item_id) AS item_count
      FROM order_details od
      ...
      WHERE od.stokvel_id = ? AND od.order_status <> 'Pending'
      GROUP BY od.order_id, od.user_id, od.stokvel_id, od.card_id, od.delivery_id,
               od.order_date, od.total_amount, od.order_status, cd.card_type,
               cd.last_four_digits, dd.delivery_status
...
export const getOrderDetailsForStokvel = async (orderId, stokvelId) => {
  const [orders] = await pool.query(
    `
      SELECT od.order_id, od.user_id, od.stokvel_id, od.card_id, od.delivery_id,
             od.order_date, od.total_amount, od.order_status, s.stokvel_name,
             cd.card_type, cd.last_four_digits, dd.delivery_address, dd.transport_type,
             dd.driver_name, dd.driver_contact, dd.delivery_date, dd.delivery_status
      FROM order_details od
      INNER JOIN stokvels s ON s.stokvel_id = od.stokvel_id
      LEFT JOIN card_details cd ON cd.card_id = od.card_id
      LEFT JOIN delivery_details dd ON dd.delivery_id = od.delivery_id
      WHERE od.order_id = ? AND od.stokvel_id = ? AND od.order_status <> 'Pending'
      LIMIT 1
    `,
    [orderId, stokvelId],
  );
  ...
  const [items] = await pool.query(
    `
      SELECT oi.order_item_id, oi.product_id, oi.supplier_price_id, oi.quantity,
             oi.unit_price, oi.subtotal, p.product_name, p.image_url, sp.supplier_name
      FROM order_items oi
      INNER JOIN products p ON p.product_id = oi.product_id
      INNER JOIN supplier_prices sp ON sp.supplier_price_id = oi.supplier_price_id
      WHERE oi.order_id = ? ORDER BY oi.order_item_id ASC
...
export const confirmPendingOrder = async (stokvelId) => {
  ...
  try {
    ...
    const [orders] = await connection.query(
      `
        SELECT od.order_id, od.user_id, od.stokvel_id, od.card_id, od.delivery_id,
               od.order_date, od.total_amount, od.order_status
        FROM order_details od
        WHERE od.stokvel_id = ? AND od.order_status = 'Pending'
    ...
    await connection.query(
      `UPDATE order_details SET order_status = 'Confirmed' WHERE order_id = ? AND order_status = 'Pending'`,
      [order.order_id],
    );
