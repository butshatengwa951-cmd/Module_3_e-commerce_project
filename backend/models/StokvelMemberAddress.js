...
const getMembership = async (userId, db = pool) => {
  ...
  if (!rows.length) throw Object.assign(new Error("You are not a member of a Stokvel."), { statusCode: 403 });
  return rows[0];
};
...
const validate = (address) => {
  for (const key of ["recipient_name", "address_line_1", "city", "province", "postal_code"]) {
    if (!address[key]) throw Object.assign(new Error("Recipient name, address, city, province and postal code are required."), { statusCode: 400 });
  }
};
...
export const getMemberAddresses = async (userId) => {
  ...
  const [[memberCount]] = await pool.query(`SELECT COUNT(*) AS member_count FROM stokvel_members WHERE stokvel_id=?`, [membership.stokvel_id]);
...
export const createMemberAddress = async (userId, input) => {
  ...
  try {
    ...
    const [existing] = await connection.query(`SELECT address_id FROM stokvel_member_addresses WHERE stokvel_member_id=? LIMIT 1`, [membership.stokvel_member_id]);
    const makeDefault = address.is_default || existing.length === 0;
    if (makeDefault) await connection.query(`UPDATE stokvel_member_addresses SET is_default=0 WHERE stokvel_member_id=?`, [membership.stokvel_member_id]);
...
export const updateMemberAddress = async (userId, addressId, input) => {
  ...
  try {
    ...
    // Never allow an account with addresses to end up without a default address.
    // If the current default is edited with is_default=false, keep it default.
    const keepDefault = Boolean(existing[0].is_default) && !address.is_default;
    if (address.is_default) await connection.query(`UPDATE stokvel_member_addresses SET is_default=0 WHERE stokvel_member_id=?`, [membership.stokvel_member_id]);
    ...
    await connection.query(
      ...
      [address.label,address.recipient_name,address.phone_number,address.address_line_1,address.address_line_2,address.city,address.province,address.postal_code,address.is_default || keepDefault ? 1 : 0,addressId,membership.stokvel_member_id]
    );
    await connection.commit();
    ...
    if (existing[0].is_default) {
      await connection.query(
        `UPDATE stokvel_member_addresses SET is_default=1
         WHERE address_id=(SELECT address_id FROM (SELECT address_id FROM stokvel_member_addresses WHERE stokvel_member_id=? ORDER BY created_at ASC,address_id ASC LIMIT 1) next_address)`,
        [membership.stokvel_member_id]
      );
    }
    await connection.commit();
...
export const setDefaultMemberAddress = async (userId, addressId) => {
  ...
  try {
    await connection.beginTransaction();
    const [existing] = await connection.query(`SELECT address_id FROM stokvel_member_addresses WHERE address_id=? AND stokvel_member_id=? LIMIT 1 FOR UPDATE`, [addressId, membership.stokvel_member_id]);
    if (!existing.length) throw Object.assign(new Error("Address not found."), { statusCode: 404 });
