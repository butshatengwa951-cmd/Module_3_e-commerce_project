...
export const getMemberDashboardData = async (userId) => {
  ...
  // Contributions are now attributed by immutable user_id. Legacy rows that
  // could not be safely mapped from member_name remain unassigned rather than
  ...
  const [walletRows] = await pool.query(`
    SELECT w.balance,
           COALESCE((SELECT SUM(t.amount) FROM stokvel_wallet_transactions t WHERE t.stokvel_id = ? AND t.transaction_type = 'CONTRIBUTION'),0) AS paid_contributions,
           COALESCE((SELECT SUM(t.amount) FROM stokvel_wallet_transactions t WHERE t.stokvel_id = ? AND t.transaction_type = 'PURCHASE'),0) AS spent_amount
    FROM stokvel_wallets w WHERE w.stokvel_id = ? LIMIT 1`, [stokvel.stokvel_id, stokvel.stokvel_id, stokvel.stokvel_id]);
  ...
  const [contributionRows] = await pool.query(`
    ...
    INNER JOIN users u ON u.user_id=mc.user_id
    INNER JOIN stokvel_members sm ON sm.user_id=u.user_id AND sm.stokvel_id=?
    WHERE mc.stokvel_id=?
