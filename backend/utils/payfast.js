...
const encodeRawPassphrase = (value) => encodeURIComponent(String(value).trim()).replace(/%20/g, "+").replace(/[!'()*]/g, (character) => `%${character.charCodeAt(0).toString(16).toUpperCase()}`);
...
export const verifyPayfastSignature=(data,rawBody=null)=>{
  const signature=String(data?.signature||"").trim();
  if(!signature)return false;
  const passphrase=process.env.PAYFAST_PASSPHRASE||null;
  let expected;
  if(rawBody){
    const pairs=String(rawBody).split("&").filter((pair)=>!/^signature=/i.test(pair));
    let parameterString=pairs.join("&");
    if(passphrase)parameterString+=`&passphrase=${encodeRawPassphrase(passphrase)}`;
    expected=crypto.createHash("md5").update(parameterString).digest("hex");
  } else {
    const withoutSignature={...data};
    delete withoutSignature.signature;
    expected=generatePayfastSignature(withoutSignature,passphrase);
  }
...
const PAYFAST_SOURCE_CIDRS=["197.97.145.144/28","41.74.179.192/27","102.216.36.0/28","102.216.36.128/28","144.126.193.139/32"];
