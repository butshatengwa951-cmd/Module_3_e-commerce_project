  ...
  try {
    const [[stats]] = await pool.query(`
      SELECT
        ...
        (SELECT COUNT(*) FROM stokvel_member_roles WHERE stokvel_role = 'CHAIRPERSON') AS chairpersons,
        (SELECT COUNT(*) FROM stokvel_member_roles WHERE stokvel_role = 'TREASURER') AS treasurers,
        ...
        (SELECT COALESCE(SUM(total_amount), 0) FROM order_details WHERE order_status = 'Completed') AS revenue,
        (SELECT COUNT(*) FROM products WHERE quantity_available <= 10) AS low_stock
    `);
    return res.json({ success: true, stats });
  } catch (error) { return sendError(res, error, "Unable to load admin dashboard."); }
};

export const getUsers = async (req, res) => {
  try {
    const [users] = await pool.query(`SELECT u.user_id,u.full_name,u.email,u.phone_number,u.role,u.created_at,GROUP_CONCAT(DISTINCT s.stokvel_name ORDER BY s.stokvel_name SEPARATOR ', ') AS stokvels FROM users u LEFT JOIN stokvel_members sm ON sm.user_id=u.user_id LEFT JOIN stokvels s ON s.stokvel_id=sm.stokvel_id GROUP BY u.user_id ORDER BY u.user_id DESC`);
    return res.json({ success: true, users });
  } catch (error) { return sendError(res, error, "Unable to load users."); }
};

export const getStokvels = async (req, res) => {
  try {
    const [stokvels] = await pool.query(`
      SELECT s.stokvel_id,s.stokvel_name,s.description,s.created_at,
        COALESCE((SELECT u.full_name FROM stokvel_members smc INNER JOIN stokvel_member_roles smr ON smr.stokvel_member_id=smc.stokvel_member_id INNER JOIN users u ON u.user_id=smc.user_id WHERE smc.stokvel_id=s.stokvel_id AND UPPER(smr.stokvel_role)='CHAIRPERSON' ORDER BY smc.stokvel_member_id LIMIT 1),'Chairperson not assigned') AS chairperson_name,
        COALESCE((SELECT u.full_name FROM stokvel_members smt INNER JOIN stokvel_member_roles smr2 ON smr2.stokvel_member_id=smt.stokvel_member_id INNER JOIN users u ON u.user_id=smt.user_id WHERE smt.stokvel_id=s.stokvel_id AND UPPER(smr2.stokvel_role)='TREASURER' ORDER BY smt.stokvel_member_id LIMIT 1),'Treasurer not assigned') AS treasurer_name,
        COUNT(sm.stokvel_member_id) AS member_count
      FROM stokvels s LEFT JOIN stokvel_members sm ON sm.stokvel_id=s.stokvel_id
      GROUP BY s.stokvel_id,s.stokvel_name,s.description,s.created_at ORDER BY s.stokvel_id
    `);
    return res.json({ success: true, stokvels });
  } catch (error) { return sendError(res, error, "Unable to load Stokvels."); }
};

export const getProducts = async (req, res) => { try { const [products] = await pool.query(`SELECT * FROM products ORDER BY product_id DESC`); return res.json({ success: true, products }); } catch (error) { return sendError(res, error, "Unable to load products."); } };
export const createProduct = async (req, res) => { try { const { product_name, description = "", category = "", image_url = "", quantity_available = 0 } = req.body; if (!product_name?.trim()) return res.status(400).json({ success:false,message:"Product name is required." }); const [result]=await pool.query(`INSERT INTO products (product_name,description,category,image_url,quantity_available) VALUES (?,?,?,?,?)`,[product_name.trim(),description,category,image_url,Math.max(0,Number(quantity_available)||0)]); const [rows]=await pool.query(`SELECT * FROM products WHERE product_id=?`,[result.insertId]); return res.status(201).json({success:true,product:rows[0]}); } catch(error){ return sendError(res,error,"Unable to create product."); } };
export const updateProduct = async (req, res) => { try { const {product_name,description,category,image_url,quantity_available}=req.body; const [result]=await pool.query(`UPDATE products SET product_name=COALESCE(?,product_name),description=COALESCE(?,description),category=COALESCE(?,category),image_url=COALESCE(?,image_url),quantity_available=COALESCE(?,quantity_available) WHERE product_id=?`,[product_name,description,category,image_url,quantity_available==null?null:Math.max(0,Number(quantity_available)||0),req.params.id]); if(!result.affectedRows)return res.status(404).json({success:false,message:"Product not found."}); const [rows]=await pool.query(`SELECT * FROM products WHERE product_id=?`,[req.params.id]); return res.json({success:true,product:rows[0]}); } catch(error){return sendError(res,error,"Unable to update product.");} };
export const deleteProduct = async (req, res) => { try { const [result]=await pool.query(`DELETE FROM products WHERE product_id=?`,[req.params.id]); if(!result.affectedRows)return res.status(404).json({success:false,message:"Product not found."}); return res.json({success:true,message:"Product deleted successfully."}); } catch(error){return sendError(res,error,"Unable to delete product. It may already be referenced by an order.");} };
...
export const getDeliveries=async(req,res)=>{try{const[deliveries]=await pool.query(`SELECT * FROM delivery_details ORDER BY delivery_id DESC`);return res.json({success:true,deliveries});}catch(error){return sendError(res,error,"Unable to load deliveries.");}};
export const updateDelivery=async(req,res)=>{try{const allowed=["Pending","In Transit","Delivered","Cancelled"],{delivery_status,driver_name,driver_contact,transport_type,delivery_date,delivery_address}=req.body;if(delivery_status&&!allowed.includes(delivery_status))return res.status(400).json({success:false,message:"Invalid delivery status."});const[result]=await pool.query(`UPDATE delivery_details SET delivery_status=COALESCE(?,delivery_status),driver_name=COALESCE(?,driver_name),driver_contact=COALESCE(?,driver_contact),transport_type=COALESCE(?,transport_type),delivery_date=COALESCE(?,delivery_date),delivery_address=COALESCE(?,delivery_address) WHERE delivery_id=?`,[delivery_status||null,driver_name,driver_contact,transport_type,delivery_date||null,delivery_address,req.params.id]);if(!result.affectedRows)return res.status(404).json({success:false,message:"Delivery not found."});return res.json({success:true,message:"Delivery updated."});}catch(error){return sendError(res,error,"Unable to update delivery.");}}
