...
export const createContributionCheckout = async (req, res) => {
  try {
    ...
    await pool.query(`INSERT INTO payfast_payments (user_id,stokvel_id,m_payment_id,amount,item_name,status) VALUES (?,?,?,?,?,'CREATED')`, [req.user.user_id, membership.stokvel_id, mPaymentId, amount.toFixed(2), itemName]);
    ...
    if (status === "COMPLETE" && payment.status !== "COMPLETE") {
      ...
      try {
        ...
        if (locked.length && locked[0].status !== "COMPLETE") {
          ...
          await connection.query(`INSERT INTO stokvel_wallet_transactions (stokvel_id,user_id,transaction_type,amount,reference_id,description) VALUES (?,?, 'CONTRIBUTION',?,?,?)`, [payment.stokvel_id, payment.user_id, payment.amount, payment.payment_id, `PayFast contribution ${payment.m_payment_id}`]);
